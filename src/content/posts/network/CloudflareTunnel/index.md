...
![进入Zero Trust](./step01-zero-trust-dashboard.png)

## 第二步：创建隧道

进去之后点左侧的"连接器"，然后点右上角的"创建隧道"。

![连接器页面](./step02-connectors-page.png)

隧道类型选 **Cloudflared**，点"选择 Cloudflared"。

![选择Cloudflared](./step03-select-cloudflared.png)

接下来给隧道起个名字，起完之后会到配置页面。这里选 **Docker**，会给你一段 docker run 命令，直接复制就行。

![获取Docker命令](./step04-docker-run-command.png)

## 第三步：在服务器上部署

SSH 到你的服务器，把刚才复制的命令粘贴进去。第一次跑会拉取镜像，等一会儿就好。

![拉取Docker镜像](./step05-pull-docker-image.png)

镜像拉完、容器跑起来之后，回到 Cloudflare Dashboard，能看到连接器状态变成"已连接"。确认没问题后点"下一步"。

![连接器已连接](./step06-connector-connected.png)
...
![配置公共主机名](./step07-public-hostname.png)
...
![Fake-IP-Filter配置](./step08-fake-ip-filter.png)
...
![fnOS Tunnel应用设置](./step09-fnos-tunnel-app.png)
