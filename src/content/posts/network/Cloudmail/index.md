...
![从Github导入Worker](./images/CreateWorker.png)
...
![创建D1数据库](./images/CreateD1db.png)

![创建KV存储](./images/CreateKV.png)

![创建R2存储桶](./images/CreateR2.png)

创建完成后，回到 Worker 的**设置 → 绑定**页面，将三个资源全部绑定上去：

> ⚠️ **绑定的时候变量名必须严格为** `db`、`kv`、`r2`，否则 Worker 无法正确读取。

![绑定KV和DB变量](./images/KVDB.png)

![绑定设置详情](./images/Binding.png)
...
![创建Resend API Key](./images/resendapi.png)
...
![添加Webhook](./images/resendaddweb.png)

![Webhook权限设置](./images/resendweb.png)

### 4. 填入 Resend Token 到 CloudMail

回到 CloudMail 的管理员后台，在设置页面中填入刚才复制的 Resend API Key：

![填入Resend Token](./images/addtoken.png)
